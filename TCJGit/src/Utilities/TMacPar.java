...
import static Jama.Matrix.random;
...
/* 
    ...
    private int maxit = 100;// maximum Iteerations
...
//    ArrayList<DenseMatrix> Y ;
    ...
    public ArrayList<Tensor> TMAC(Tensor tensor,ArrayList<DenseMatrix> X,ArrayList<DenseMatrix> Y, ArrayList<TensorIndex> known,ArrayList<Double> data ){
...
System.out.println("<<<<<<<<<<<<<<<<<<<<<<< TMac Par:  >>>>>>>>."+mapLinearTI.toString());
...
//for (int i = 0; i < N; i++)
//    io.write2DenseMatrix( X.get(i),"BeforeX"+i+".csv");
//
//for (int i = 0; i < N; i++)
//    io.write2DenseMatrix(Y.get(i),"BeforeY"+i+".csv");
...
// if coreNway(n) < rank_max(n) && reschg(n) < reschg_tol
                           rank_inc_adaptive(M,i,NwayDV,X,Y);                              //rank_inc_adaptive();
...
//% adaptively update weight
            ...
            M0 = M.Copy();
            res0 = res;
//            System.out.println(" relerr1 = " + relerr1);
//            System.out.println(" relerr2 = " + relerr2);
//            System.out.println(" Totalres = " + TotalRes);
       ...
       IO io = new IO();
         for (int i = 0; i < N; i++)
        io.write2DenseMatrix( X.get(i),"Xj"+i+".csv");

    for (int i = 0; i < N
            ; i++)
    io.write2DenseMatrix(Y.get(i),"Yj"+i+".csv");
        ...
        io.write2DenseMatrix(XtimesY1,"XYj1.csv");
        io.write2DenseMatrix(XtimesY2,"XYj2.csv");
        io.write2DenseMatrix(XtimesY3,"XYj3.csv");
      ...
      Tensor folding2 = matlab.Fold(XtimesY2, NwayDV, 1);
        ...
        io.writeTensor(folding1, "F1.csv");
        io.writeTensor(folding2, "F2.csv");
        io.writeTensor(folding3, "F3.csv");
        
        System.out.println("Length  = "+ folding1.getTensor()[0][0].length);
        ...
        ArrayList<Double> DH = matlab.DehankelizationR(kMatrix1, 64, 0.5);
    io.writeArrayList2File(DH, "_"+matq+"_rec1.csv");
    
          DenseMatrix kMatrix2 = folding2.getKMatrix(i);
        ArrayList<Double> DH2 = matlab.DehankelizationR(kMatrix2, 64, 0.5);
    io.writeArrayList2File(DH2, "_"+matq+"_rec2.csv");
        
    
          DenseMatrix kMatrix3 = folding3.getKMatrix(i);
        ArrayList<Double> DH3 = matlab.DehankelizationR(kMatrix3, 64, 0.5);
    io.writeArrayList2File(DH3, "_"+matq+"_rec3.csv");
        }
   

        
//        Tensor add1 = folding1.TensorAdd(folding1, folding2);
//        Tensor finale = add1.TensorAdd(add1, folding3);
//         finale.copyTensorObserved(data, known);

        
        System.out.println("reller1 = "+relerr1);
        System.out.println("reller2 = "+relerr2);
...
//    }
        ...
        if(i==0) rdnx = io.readDenseMatrix((int)coNway.getEntry(0,i), 1, "", "rdnx1.csv");
        else if(i==1)  rdnx = io.readDenseMatrix((int)coNway.getEntry(0,i), 1, "", "rdnx2.csv");
        else rdnx = io.readDenseMatrix((int)coNway.getEntry(0,i), 1, "", "rdnx3.csv");
     ...
     public void rank_2(Tensor t,int i, DenseVector NwayDV,ArrayList<DenseMatrix> X,ArrayList<DenseMatrix> Y) {

    // System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< rank_inc_ad "+i);
        Matlab matlab = new Matlab();
        IO io = new IO();
        Jama.QRDecomposition QR = new Jama.QRDecomposition(matlab.LAML2JAMAMatrix(Yt.get(i)));
...
//            String path  = "C:\\Users\\Roubakas\\Documents\\NetBeansProjects\\Main\\Data\\";
//            if (i == 0) rdnxList = io.ReadVars(path, "rdnx1.csv");
//            else if (i == 1) rdnxList = io.ReadVars(path,"rdnx2.csv");
//            else rdnxList = io.ReadVars(path,"rdnx3.csv");
//           
//            int rdnxCounter = 0;
//            for (int zz = 0; zz < coNway.getEntry(0,i); zz++) {
//                rdnx.setEntry(zz, 0, rdnxList.get(rdnxCounter));
//                rdnxCounter++;
//            }


            //  TODO DenseMatrix rdnx = (DenseMatrix) ml.utils.Matlab.rand((int) coNway.get( i), 1); // rdnx = randn(coNway(n),1);
            // 
            DenseMatrix mul1 = (DenseMatrix) ml.utils.Matlab.mtimes(Qt, rdnx);
            DenseMatrix mul2 = (DenseMatrix) ml.utils.Matlab.mtimes(Q, mul1);
    ...
    public double MSE(Tensor original, Tensor reconstructed ){
        ...
        int sum_sq = 0;
        double mse;
double[][][] values = original.getTensor();
        int h = values.length;
        int w = values[0].length;
        int o = values[0][0].length;
        for (int i = 0; i < h; ++i) {
            for (int j = 0; j < w; ++j) {
                for (int k = 0; k < o; k++) {
                    double p1 =original.getTensor()[i][j][k];
                    double p2 = reconstructed.getTensor()[i][j][k];
                    double err = p2 - p1;
                    sum_sq += (err * err);
                }
            }
        }
        mse = (double)sum_sq / (h * w * o);
